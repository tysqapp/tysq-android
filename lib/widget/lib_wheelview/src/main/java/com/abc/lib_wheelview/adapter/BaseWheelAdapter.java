 ...
 */
package com.abc.lib_wheelview.adapter;

import android.view.View;
import android.view.ViewGroup;
import android.widget.BaseAdapter;

import com.abc.lib_wheelview.widget.IWheelView;
...
public abstract class BaseWheelAdapter<T> extends BaseAdapter {
    ...
    public boolean isEnabled(int position) {
        if (mClickable) {
            if (mLoop) {
                if (position % mList.size() == mCurrentPosition) {
                    return true;
                }
            } else {
                if (position == (mCurrentPosition + mWheelSize / 2)) {
                    return true;
                }
            }
        }
